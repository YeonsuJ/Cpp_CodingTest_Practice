...
int toSeconds(string t){
    int idx = t.find(':'); 
    int min = stoi(t.substr(0, idx));
    int sec = stoi(t.substr(idx+1));
    return min*60+sec;
}

// 오프닝 구간인지 확인
void jumpOpening(int& cur, int opst, int oped){
    if(cur >= opst && cur <= oped)
        cur = oped; // 건너뛰기
}

string toTime(int cur){
    int min = cur / 60;
    int sec = cur % 60;
...
string solution(string video_len, string pos, string op_start, string op_end, vector<string> commands) {
    ...
    int opst = toSeconds(op_start);
    int oped = toSeconds(op_end);
    
    jumpOpening(cur, opst, oped);
    
    for(const string& cmd : commands){
        if (cmd == "next") {
            cur += 10; 
            if (cur > vlen) cur = vlen; // 영상 길이 초과 방지
        } 
        else if (cmd == "prev") {
            cur -= 10;
            if (cur < 0) cur = 0; // 0초 미만 방지
        }
...
## 분석 및 배운점
- Lv.1 구현(Implement) 문제 (소요시간 : 56분)
- 초<-> 분 변환, 명령어 이동 후에도 오프닝 재확인 필요성 등에서 시간이 다소 소요됨.

- 문자열 <-> 정수형 변환 및 파싱 
- 데이터 정규화 (시간(시분초), 날짜(연월일), 진법 변환 등) 문제에 익숙해져야함.
