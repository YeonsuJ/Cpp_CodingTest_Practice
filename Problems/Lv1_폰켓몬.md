...
int solution(vector<int> nums) {
    ...
    int unique_count = 1; // 첫 번째 폰켓몬은 무조건 한 종류이므로 1부터 시작
    for (int i = 1; i < nums.size(); i++) {
        // 이전 요소와 값이 다를 때만 새로운 종류로 카운트
        if (nums[i] != nums[i - 1]) {
            unique_count++;
        }
    }
...
## 접근 및 배운점 1
- 배열을 정렬하여 같은 종류를 옆에 붙이고, 이때 값이 달라지는 순간만 체크하여 종류의 수 카운트
- 시간복잡도 : O(N log N)
...
int solution(vector<int> nums) {
    ...
    // unordered_set을 사용하여 중복을 제거
    unordered_set<int> pokemon_types(nums.begin(), nums.end());
    
    int unique_count = pokemon_types.size();
    
    // 종류의 수와 max_pick 중 작은 값을 반환
    return min(unique_count, max_pick);
}
```
</details>
...
## 접근 및 배운점 2
- unordered_set으로 중복제거 (unordered_set 은 해시 테이블 기반으로 동작하며, 삽입 시 중복된 값은 자동으로 무시됨)
- 평균시간 복잡도 : O(N)
